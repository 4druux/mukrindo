...
exports.createNotificationRequest = async (req, res) => {
  try {
    ...
    const newRequest = new NotificationRequest({
      brand,
      model,
      year,
      phoneNumber,
...
exports.getAllNotificationRequests = async (req, res) => {
  try {
    const requests = await NotificationRequest.find().sort({ createdAt: -1 });
    res.status(200).json({
      success: true,
...
exports.getNotificationRequestById = async (req, res) => {
  try {
    const request = await NotificationRequest.findById(req.params.id);
    if (!request) {
...
exports.updateNotificationStatus = async (req, res) => {
  try {
    ...
    const updatedRequest = await NotificationRequest.findByIdAndUpdate(
      id,
...
// @access  Private (Admin)
exports.deleteNotificationRequest = async (req, res) => {
  try {
    const request = await NotificationRequest.findByIdAndDelete(req.params.id);
